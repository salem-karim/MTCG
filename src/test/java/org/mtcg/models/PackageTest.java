...
public class PackageTest {
  ...
  public void tooManyCards() {
    ...
    Card[] tooManyCards = Stream.concat(Stream.of(cards), Stream.of(extraCards))
        .toArray(Card[]::new);
  ...
  public void exactPackageSize() {
    ...
    Card extraCard = new Card(UUID.randomUUID(), "Extra Card", 25.0f, CardType.MONSTER, Element.NORMAL);

    Card[] exactCards = Stream.concat(Stream.of(cards), Stream.of(extraCard))
        .toArray(Card[]::new);

    // Test for exactly 5 cards (should not throw exception and return a valid
    // Package object)
    Package pkg = new Package(exactCards, UUID.randomUUID());
